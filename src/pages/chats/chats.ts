...
export class ChatsPage {
  ...
  notifyMessages(msgs: Array<Message>) {
    this.translate.get(['new_msg', 'new_msgs', 'new_msg_from', 'new_msgs_from']).subscribe(value => {
      this.oneSignal.postNotification({
        ...
        contents: { en: (((msgs.length > 1) ? value['new_msgs_from'] : value['new_msg_from']) + " " + this.userMe.first_name) },
        data: { msgs: msgs }
